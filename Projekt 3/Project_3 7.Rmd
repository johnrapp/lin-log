...
models <- list(
  ...
  glm(survived ~ sex + age + pclass + log.fare + siblings.spouses, data = titanic, family = "binomial"),
...
The two models considered out of these are some interaction 1 and the s+a+p+f+s. 
...
Down below, a comparison of the two  models unfolds dadada.
...
```{r}
 dfbeta.plot(model.no.parents, titanic, 1, 0)
 dfbeta.plot(model.interaction, titanic, 0, 1)
...
#Optimal model
```{r}
optimal.model <- glm(survived ~ sex + log.fare + age + pclass + siblings.spouses, data = titanic, family = "binomial")
(sum <- summary(optimal.model))
calc.metrics(optimal.model)

coeff <- sum$coefficients[, "Estimate"]

impact <- data.frame(
  female = coeff[2],
  fare = coeff[3]*median(titanic$log.fare),
  age = coeff[4]*median(titanic$age),
  pclass2 = coeff[5],
  pclass3 = coeff[6],
  siblings.spouses = coeff[7]*mean(titanic$siblings.spouses)
)

abs(impact$fare / impact$female)
abs(impact$age / impact$female)
abs(impact$pclass2 / impact$female)
abs(impact$pclass3 / impact$female)
abs(impact$siblings.spouses / impact$female)
