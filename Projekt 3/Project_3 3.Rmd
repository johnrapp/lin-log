...
```{r}
model.no.parents <- glm(survived ~ sex + log.fare + age + pclass + siblings.spouses, data = titanic, family = "binomial")

model.interaction <- glm(survived ~ log.fare * age * sex * is.alone * pclass, data = titanic, family = "binomial")
...
(mertic <- calc.metrics.models(best.models, names))




qq(model.no.parents, "no parents QQ", "Theoretical Quantiles", "Sample Quantiles")
qq(model.interaction, "interaction QQ", "Theoretical Quantiles", "Sample Quantiles")
...
```{r}
cook.plotter(model.no.parents, titanic, "Cook's distance against the liner predictor - No Parents")
cook.plotter(model.interaction, titanic, "Cook's distance against the liner predictor - Interaction")
...
```{r}
dfbeta.plot(model.no.parents, titanic, 0)
dfbeta.plot(model.interaction, titanic, 1)


```

From the DFbeta plots it looks like the all.interaction has several issues. For the DFbeta.parents.children there are several points that are with quite the marignal above the horizontal lines. For the all model and the some.interaction model, The points in the plots are a lot more centered around the x axis, and the outliers are not as far away, (Fewer and closer). In the all.interaction model we want to look at the data entires that causes DF-beta values above 0.5. ## <--- change


Outliers for the no parents 
```{r}
dfb1 <- dfbetas(model.no.parents)

#dfb
d1.Intercept <- as.vector(dfb1[,1])
d1.log.fare <- as.vector(dfb1[,2])
d1.age <- as.vector(dfb1[,3])
d1.sexfemale <- as.vector(dfb1[,4])
d1.pclass <- as.vector(dfb1[,5])
d1.siblings.spouses <- as.vector(dfb1[,6])

n = 2 / sqrt(nrow(titanic))


(outlier.index1a <- which(abs(d1.Intercept) > n))
(outlier.index2a <- which(abs(d1.log.fare) > n)) 
(outlier.index3a <- which(abs(d1.age) > n))
(outlier.index4a <- which(abs(d1.sexfemale) > n))
(outlier.index5a <- which(abs(d1.pclass) > n))
(outlier.index6a <- which(abs(d1.siblings.spouses) > n))  


outlier.index1 <- c(outlier.index1a, outlier.index2a, outlier.index3a, outlier.index4a, outlier.index5a, outlier.index6a)
(outlier.index1 <- unique(outlier.index1))
...
Outliers for the interaction 
```{r}
dfb2 <- dfbetas(model.interaction)
#dfb
d.Intercept2 <- as.vector(dfb2[,1])
d.log.fare2 <- as.vector(dfb2[,2])
d.age2 <- as.vector(dfb2[,3])
d.sexfemale2 <- as.vector(dfb2[,4])
d.is.alone <- as.vector(dfb2[,5])
d.pclass2 <- as.vector(dfb2[,6])
d.pclass3 <- as.vector(dfb2[,7])
d.siblings.spouses2 <- as.vector(dfb2[,6])

n = 2 / sqrt(nrow(titanic))


(outlier.index1b <- which(abs(d.Intercept2) > n))
(outlier.index2b <- which(abs(d.log.fare2) > n)) 
(outlier.index3b <- which(abs(d.age2) > n))
(outlier.index4b <- which(abs(d.sexfemale2) > n))
(outlier.index5b <- which(abs(d.pclass2) > n))

outlier.index2b <- c(outlier.index1b, outlier.index2b, outlier.index3b, outlier.index4b, outlier.index5b)
(outlier.index2b <- unique(outlier.index2b))

out <- which(abs(dfb2) > 0.5)
...
```{r}
cooks <- cooks.distance(model.interaction)
outliers.cooks <- which(cooks > 1)
outlier.cooks
```
...
```{r}
liz <- data.frame(sex = "female", log.fare =4.11, age= 32, siblings.spouses = 1, pclass = "2")
calc.pred.prob(model.no.parents, liz)

jack <- data.frame(sex = "male", log.fare =2.77, age= 19, siblings.spouses = 0, pclass = "3")
calc.pred.prob(model.no.parents, jack)


rose <- data.frame(sex = "female", log.fare =5.3, age= 17, siblings.spouses = 1, pclass = "1")
calc.pred.prob(model.no.parents, rose)


albert <- data.frame(sex = "male", log.fare =5.3, age= 4, siblings.spouses = 0, pclass = "3")
calc.pred.prob(model.no.parents, albert)

molly <- data.frame(sex = "female", log.fare =4.39, age= 60, siblings.spouses = 1, pclass = "1")
calc.pred.prob(model.no.parents, molly)
