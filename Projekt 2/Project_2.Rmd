...
As seen in the table 2, all of the P-values are < 0.05, meaning both that conties with no `higrads` has greater than 0 probability of having a high crime rate, and that `higrads` has a statistically significant effect on `hicrm`. This effect can be measured by looking at $e^{\beta_{higrads}}$, showing that an increase of 1% in `higrads` decreases odds of `hicrm` by `r display.percent(decrease.one.percent[1])`% (confidence interval [`r display.percent(decrease.one.percent[2])`%, `r display.percent(decrease.one.percent[3])`%]), while an increase of 10% decreases odds of by `r display.percent(decrease.ten.percent[1])`% (confidence interval [`r display.percent(decrease.ten.percent[2])`%, `r display.percent(decrease.ten.percent[3])`%]).
...
anova(model.0, model.south)
anova(model.0, model.northeast)
anova(model.0, model.midwest)
anova(model.0, model.west)
anova(model.0, model.full)
