...
```{r setup_pdf, eval=FALSE, include = FALSE}
# Install from CRAN
...
table.beta.ci <- cbind("Estimate" = coeff.beta, ci.beta)
row.names(table.beta.ci) <- c("$\\beta_0$", "$\\beta_1$")

kable(table.beta.ci, caption = "Test")
...
table.beta.p <- data.frame(p = sum$coefficients[, "Pr(>|z|)"])
row.names(table.beta.p) <- c("$\\beta_0$", "$\\beta_1$")

kable(table.beta.p, col.names = "P-value", caption = "Test")
