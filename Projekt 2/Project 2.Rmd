...
model <- glm(hicrm ~ region, data = cdi, family = "binomial")
sum <- summary(model)
...
ci.prob <- cbind(lo = pred$fit - standard.error * pred$se.fit, 
                    hi = pred$fit + standard.error * pred$se.fit)
...
# pred.counties
# 
# table(pred.counties$region, pred.counties$prob.hicrm)
# 
# num.hicrm <- sum(pred.counties$hicrm)
# 
# # Sensitivity
...
# 
# 
# #Estimate the intervals as in the PP
# beta.hats <- c(coeff.beta[1], coeff.beta[1] + coeff.beta[2], coeff.beta[1] + coeff.beta[3], coeff.beta[1] + coeff.beta[4])
# 
# beta.hats
# 
# p.ests  <- data.frame(name = c("Northeast", "Souht", "Midwest", "West"),
#                       n0 = c(cross.table[1,1], cross.table[2,1], cross.table[3,1], cross.table[4,1]),
#                       n1 = c(cross.table[1,2], cross.table[2,2], cross.table[3,2], cross.table[4,2]))
# p.ests$n <- p.ests$n0 + p.ests$n1
# p.ests$phat <- p.ests$n1 / p.ests$n
# p.ests$npq <- p.ests$n * p.ests$phat * (1 - p.ests$phat)
# p.ests$se <- sqrt(p.ests$phat * (1 - p.ests$phat) / p.ests$n)
# p.ests$lo <- p.ests$phat - 1.96 * p.ests$se
# p.ests$hi <- p.ests$phat + 1.96 * p.ests$se
# p.ests
# 
# 
# #CI for the log-odds
# se.b0 <- sqrt(1 / p.ests$n0[1] + 1 / p.ests$n1[1])
# se.b1 <- sqrt(1 / p.ests$n0[1] + 1 / p.ests$n1[1] + 1 / p.ests$n0[2] + 
#                 1 / p.ests$n1[2])
# se.b2 <- sqrt(1 / p.ests$n0[1] + 1 / p.ests$n1[1] + 1 / p.ests$n0[3] + 
#                 1 / p.ests$n1[3])
# se.b3 <- sqrt(1 / p.ests$n0[1] + 1 / p.ests$n1[1] + 1 / p.ests$n0[4] + 
#                 1 / p.ests$n1[4])
...
#                  model$coefficients[4] + 1.96 * se.b2 *c(-1, 1))
# ci.beta
# 
# 
# exp(ci.beta)
