...
Title: The higrad model

The first model considered had higrads as the sole covariate. In order to determine if there is a relationship between hicrm and higrads they are plotted against each other, see figure XX. Because hicrm is a categorical varible it is very difficult to determine if there is a relationship by the pattern of the plot. In order to circumvent this problem a kernel smoother was added to the plot. The most smooth looking line was attained by setting the bandwidth to 20. The kernel regresion was sort of S-shaped meaning that it has a negative B1. Furthermore the fitted model along with it's 95 % confidence interval was added to the plot. The kernel smoother and fitted model is also found in figure XX.
...
As can be seen in figure XX, a higher number of higrads seems to make a county less probable to qualify as high crime county. Logically this makes sense. THe B!! values together with their 95 % confidence inteval is presented in table XX. Neither one of the B! confidence interval cover zero meaning that they are statistically significant at a! = 0.05. This is verfied by the very small P value.  
...
kable(table.beta.ci, caption = "Test", digits = c(3,3,3,4))
...
Next, we try a model where we try to predict crime rate based on region. Since region is not a continous variable, but a categorial variable, it is modelled using "dummy variables" $X_i$. In order to implement this effectively, one of the categories is chosen as a reference variable, and the effects of other categories are measured in comparison to it.
...
kable(cross.table, col.names = c("Low crime", "High crime"), caption = "Test", label = "cross-tabulation")
...
As a reference region, the one that has the largest number of counties in it’s smallest low/high category was chosen. As a tie-breaker, the other low/high category was used. This approach produces the lowest standard error, and therefore highest significance.

As seen in table \ref{tab:cross-tabulation}, `r reference.level` was chosen according to those contstraints.


Using this reference region, the model looks like
\begin{equation}
...
Where $epsilon \sim N(0, \sigma)$ i.i.d.

Where then the \beta coefficients are measured relative to `r reference.level`, and \beta_0 is coefficient for `r reference.level`.


#Results
Next, a logistic regression model is fit with the given data set, in order to study significance. In table \ref{tab:region_beta}, the significance of the \beta coefficients are studied
