...
# ### a "logical" plot-order:
# ordning <- c(1, 2, 3, 4, 5, 8, 7, 6, 9, 10, 11, 12)
# samlat[ordning, ]
# 
# ### create the plots on page 15 and 16:
# antal <- nrow(samlat)
# x <- 1:antal
# with(samlat[ordning,], {
#   plot(x, rep(0, antal), type = "n", ylim = c(0, 1), xaxt = "n",
#        ylab = "R2/R2-adjusted", xlab = "model",
#        main = "Coefficient of determination")
#   axis(1, at = x, labels = modname)
#   lines(x, r2, col = "red", lty = 2, lwd = 2)
#   lines(x, r2adj, col = "blue", lty = 1, lwd = 2)
#   points(x[sig], r2adj[sig], col = "blue", pch = 16)
#   points(x[sig], r2[sig], col = "red", pch = 16)
# })
# legend(1, 1, c("R2", "R2-adj", "all sign."),
#        col = c("red", "blue", "black"),
#        lty = c(2, 1, 0), pch = c(NA, NA, 16), lwd = c(2, 2, 0))
# 
# ## check that AIC and BIC fit withing the min-max-limits:
# summary(samlat)
# with(samlat[ordning,], {
#   plot(x, rep(0, antal), type = "n", ylim = c(75, 95), xaxt = "n",
#        ylab = "AIC and BIC", xlab = "model",
#        main = "Information criteria")
#   axis(1, at = x, labels = modname)
#   lines(x, aic, col = "red", lty = 2, lwd = 2)
#   lines(x, bic, col = "blue", lty = 1, lwd = 2)
#   points(x[sig], aic[sig], col = "red", pch = 16)
#   points(x[sig], bic[sig], col = "blue", pch = 16)
# })
# legend(9, 95, c("AIC", "BIC", "all sign."),
#        col = c("red", "blue", "black"),
#        lty = c(2, 1, 0), pch = c(NA, NA, 16), lwd = c(2, 2, 0))
...
xb <- predict(model.region.higrads)


cdi$v <- influence(model.region.higrads)$hat

#Pearson Standardized residuals
cdi$r <- influence(model.region.higrads)$pear.res / sqrt(1 - influence(model.region.higrads)$hat)
summary(cdi$r)
...
cdi$ds <- influence(model.region.higrads)$dev.res / sqrt(1 - influence(model.region.higrads)$hat)
summary(cdi$ds)
with(cdi, plot(ds ~ xb, 
...
#Cook's
cdi$cook <- cooks.distance(model.region.higrads)
summary(cdi$cook)
