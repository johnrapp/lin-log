...
#Pseudo R2
#R2.col <- t(cbind(pR2(model.higrads)[6], pR2(model.region)[6], pR2(model.both)[6]))
...
anova(model.region.higrads, model.interaction)
...
model.names1 <- c("H", "H + R", "H + R + Poors", "H + R + Poors + Phys1000")
models1 <- list(model.1, model.2, model.3, model.4)

models1

aic1 <- do.call(AIC, models1)
#calc.aic <- AIC(models)

calc.aic1 <- do.call(AIC, models1)$AIC
calc.bic1 <- do.call(AIC, c(models1, list(k = log(nrow(cdi)))))$AIC
     

metrics1 <- sapply(models1, calc.metrics)

#Pseudo R2
#R2.col <- t(cbind(pR2(model.higrads)[6], pR2(model.region)[6], pR2(model.both)[6]))
#colnames(R2.col)[colnames(R2.col)=="r2CU"] <- "Pseudo R2"

pseudo.R21 <- sapply(models1, pR2)["r2CU",]

compare.models1 <- data.frame(
  model = model.names1,
  aic = calc.aic1,
  bic = calc.bic1,
  sensitivity = unlist(metrics1["sensitivity",]),
  specificity = unlist(metrics1["specificity",]),
  PseudoR2 = pseudo.R21
)
...
topmodel <- glm(hicrm ~ region + poors + phys1000, data = cdi, family = "binomial")

aicTop <- AIC(topmodel)
bicTop <- AIC(topmodel, k = log(nrow(cdi)))
sensitivityTop <- calc.metrics(topmodel)[1]
specificityTop <- calc.metrics(topmodel)[2]
R2pseudoTop <- pR2(topmodel)[6]
 interaction.data1 <- cbind("aic" = aicTop, "bic" = bicTop, "sensitivity" = sensitivityTop, "spedificity" = specificityTop, "Pseudo R2" = R2pseudoTop)
